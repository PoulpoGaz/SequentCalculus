...
let () =
  let seq1 = create_sequent [] [Or(V 1, V 1)]
  and seq2 = create_sequent [] [Impl(Impl(Impl(V 1, V 2),V 1),V 2)]

  and seq3 = create_sequent [] [Or(V 1, Not(V 1))]
  and seq4 = create_sequent [] [Impl(Impl(Impl(V 1, V 2),V 1),V 1)]
  and seq5 = create_sequent [And(And(V 1, V 2), V 3)] [And(V 1, And(V 2, V 3))]
  and seq6 = create_sequent [Or(Or(V 1, V 2), V 3)] [Or(V 1, Or(V 2, V 3))]
  and seq7 = create_sequent [Impl(V 1, V 2)] [Impl(Not(V 2), Not(V 1))]
  and seq8 = create_sequent [Impl(Not(V 2), Not(V 1))] [Impl(V 1, V 2)]
  and seq9 = create_sequent [V 1; V 2] [Impl(V 3, And(V 1, V 3))] in
  ...
  let seq10 = create_sequent [] [
    Impl (
      Impl ( Impl ( V 1, Impl (V 2, V 3)),V 3),    (*  ((a → b → c) → c)  *)
      Impl (
        Impl (
          Impl (
            Impl (V 1, V 3) , V 3),
            Impl (Impl (Impl (V 2, V 3), V 3), V 3)
          ),
        V 3
      )
    )
  ...
  let to_string = function i -> match i with
    | 1 -> "\\varphi_1"
    | 2 -> "\\varphi_2"
    | 3 -> "\\varphi_3"
  ...
  proof_search_latex out to_string seq1 |> Printf.printf "%B\n";
  proof_search_latex out to_string seq2 |> Printf.printf "%B\n";

  proof_search_latex out to_string seq3 |> Printf.printf "%B\n";
  proof_search_latex out to_string seq4 |> Printf.printf "%B\n";
  proof_search_latex out to_string seq5 |> Printf.printf "%B\n";
  proof_search_latex out to_string seq6 |> Printf.printf "%B\n";
  proof_search_latex out to_string seq7 |> Printf.printf "%B\n";
  proof_search_latex out to_string seq8 |> Printf.printf "%B\n";
  proof_search_latex out to_string seq9 |> Printf.printf "%B\n";
  proof_search_latex out to_string seq10 |> Printf.printf "%B\n";
